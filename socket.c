...
int dispatch_main_socket(server_t *server)
{
    ...
    //绑定端口
    if(-1 == bind(main_socket->listenfd, (struct sockaddr *)&main_socket_addr, sizeof(struct sockaddr_in)))
	{
    ...
    {   
		log_error("add listenfd %d to main epfd %d fail", main_socket->listenfd, main_socket->epfd);
        return -1; 
    }   
